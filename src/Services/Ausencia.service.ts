...
import type { ausencia } from '../Models/Ausencia';
...
export class AusenciaService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): ausencia {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f.Title,
  ...
  // ---------- CRUD ----------
  async create(record: Omit<ausencia, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<ausencia, 'ID'>>) {
    await this.ensureIds()
