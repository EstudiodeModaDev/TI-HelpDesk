...
export class DispositivosService {
  ...
  constructor(
    ...
    listName = 'Prestamos - Dispositivos'
  ) {
    this.graph = graph;
  ...
  private toModel(item: any): dispositivos {
    const f = item?.fields ?? {};
    return {
  ...
  // ---------- CRUD ----------
  async create(record: Omit<dispositivos, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<dispositivos, 'ID'>>) {
        await this.ensureIds();
  ...
  async getAll(opts?: GetAllOpts): Promise<PageResult<dispositivos>> {
    await this.ensureIds();
  ...
  async getByNextLink(nextLink: string): Promise<PageResult<dispositivos>> {
    return this.fetchPage(nextLink, /*isAbsolute*/ true);
  }

  private async fetchPage(url: string, isAbsolute = false): Promise<PageResult<dispositivos>> {
