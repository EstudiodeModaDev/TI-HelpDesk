...
export function usePrestamos() {
  ...
  const handleSubmit = async (): Promise<{continue: boolean, created: prestamos | null}> => {
    ...
    try {
      const ticket =await Tickets.create({
        Title: "Prestamo de equipo a " + state.nombreSolicitante, 
        ANS: "N/A", 
        Categoria: "Otros", 
  ...
  const finalizeLoan = async (loan: prestamos, continuar: boolean) => {
    try {
      ...
      await Tickets.update(loan.IdTicket, {Estadodesolicitud: "Cerrado", TiempoSolucion: toISODateTimeFlex(new Date()),});
