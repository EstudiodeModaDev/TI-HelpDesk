...
export function useDetallado(TicketsSvc: TicketsService) {
  ...
  const [conteoPorMes, setConteoPorMes] = React.useState<ConteoMes[]>([]) // NUEVO

  // =========================
  // Filtro base (usa range; por defecto = mes en curso)
  // =========================
  const buildFilterTickets = React.useCallback((): GetAllOpts => {
    ...
    // Siempre filtra por rango (ya está seteado al mes en curso por defecto)
    if (fromD && toD && fromD <= toD) {
      filters.push(`fields/FechaApertura ge '${fromD}T00:00:00Z'`)
      filters.push(`fields/FechaApertura le '${toD}T23:59:59Z'`)
    } else {
      // Fallback (no debería ocurrir): usa mes en curso
      const now = new Date()
  ...
  // =====================================================================
  // NUEVO: obtenerConteoUltimosMeses
  // - Trae tickets de los últimos `months` meses (incluye el actual)
  // - Agrupa por 'YYYY-MM' y rellena con 0 los que no tengan casos
  // - Guarda en `conteoPorMes`
  // =====================================================================
  const obtenerConteoUltimosMeses = React.useCallback(
    async (months: number): Promise<ConteoMes[]> => {
      ...
      try {
        ...
        // Construye todos los meses de la ventana con ceros donde falte
        const out: ConteoMes[] = []
  ...
  const obtenerTotal = React.useCallback(async () => {
    ...
    try {
      ...
      for (const t of tickets) {
        const st = normS((t as any)?.Estadodesolicitud || (t as any)?.fields?.Estadodesolicitud)
