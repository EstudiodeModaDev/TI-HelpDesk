...
import {
  ...
  useGraphServices,
} from "./graph/GrapServicesContext";
...
const NAV: MenuItem[] = [
  {
    id: "home",
    label: "Home",
    icon: <img src={HomeIcon} alt="" className="sb-icon" />,
    to: <DashBoardPage />,
  ...
  },
  {
    ...
    label: "Nuevo Ticket",
    icon: <img src={addIcon} alt="" className="sb-icon" />,
    to: <NuevoTicketUsuarioForm />,
    roles: ["Usuario", "Jefe de zona", "Listo"],
  },
  {
    id: "ticketTable",
    label: "Ver Tickets",
    icon: <img src={seeTickets} alt="" className="sb-icon" />,
    to: <TablaTickets />,
    autocollapse: true,
  },
  {
    id: "task",
    label: "Tareas",
    icon: <img src={tareasIcon} alt="" className="sb-icon" />,
    to: <TareasPage />,
    roles: ["Administrador", "Tecnico", "Listo"],
    autocollapse: true,
  },
  ...
  },
  {
    id: "formatos",
    label: "Formatos",
    icon: <img src={filesIcon} alt="" className="sb-icon" />,
    to: <Formatos />,
  ...
  },
  {
    id: "info",
    label: "Información",
    icon: <img src={infoIcon} alt="" className="sb-icon" />,
    to: <InfoPage />,
    roles: ["Administrador", "Tecnico", "Listo"],
  },
  {
    ...
    icon: <img src={storageIcon} alt="" className="sb-icon" />,
    to: <StoragePage />,
    roles: ["Administrador", "Listo"],
  },
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        id: "tips",
        label: "Tips",
        icon: <img src={infoIcon} className="sb-icon" />,
        to: <AnnouncementsTable />,
        roles: ["Administrador", "Tecnico", "Listo"],
      },
      {
        id: "reports",
  ...
  },
  {
    id: "acciones",
    label: "Acciones",
    icon: <img src={ActionsIcon} className="sb-icon" />,
    roles: ["Administrador", "Tecnico", "Jefe de zona", "Listo"],
    children: [
      {
        id: "siesa",
        label: "Siesa",
        roles: ["Administrador", "Tecnico", "Jefe de zona", "Listo"],
        icon: <img src={siesaIcon} className="sb-icon" />,
        children: [
          {
            id: "cajpos",
            label: "Cajeros POS",
            icon: <img src={cajerosIcon} className="sb-icon" />,
            to: (rctx: RenderCtx) =>
              rctx.services ? (
                <CajerosPOSForm
                  services={{
                    Tickets: rctx.services.Tickets,
                    Logs: rctx.services.Logs,
                  }}
                />
              ) : (
...
function firstLeafId(nodes: readonly MenuItem[]): string {
  const pick = (n: MenuItem): string =>
    n.children?.length ? pick(n.children[0]) : n.id;
  ...
  React.useEffect(() => {
    ...
    const walk = (nodes: readonly MenuItem[], path: string[] = []) => {
      nodes.forEach((n) => {
        const p = [...path, n.id];
        if (n.id === selected)
          p.slice(0, -1).forEach((id) => (next[id] = true));
      ...
      <div className="sidebar__footer">
        ...
        {!collapsed && (
          <div className="sb-prof__info">
            <div className="sb-prof__mail">
              {user?.mail || "usuario@empresa.com"}
            </div>
...
function LoggedApp({ user }: { user: User }) {
  ...
  const element = React.useMemo(() => {
    ...
    if (typeof item.to === "function") {
      return (item.to as (ctx: RenderCtx) => React.ReactNode)({
        services: {
  ...
  const handleSelect = React.useCallback(
    (id: string) => {
      ...
      if (
        it.autocollapse &&
        (isNarrow || true) /* quita esto si quieres sólo en móvil */
      ) {
        setCollapsed(true);
        try {
  ...
  return (
    <div
      className={`page layout layout--withSidebar ${collapsed ? "is-collapsed" : ""}`}
    >
      <Sidebar
        navs={navs}
        selected={selected}
      ...
      <main className="content content--withSidebar">
        <div className="page-viewport">
          ...
          >
            <button
              className="btn btn-transparent-final btn-s"
              onClick={toggle}
            ...
            {(role === "Tecnico" || role === "Administrador") && (
              <button
                className="btn btn-transparent-final btn-m"
                onClick={() => setAusencia(!ausencia)}
              ...
              >
                ...
                >
                  <path
                    fill="#000000"
                    d="M512 1025q-104 0-199-40.5t-163.5-109T40.5 712T0 513t40.5-199t109-163.5T313 41T512 0t199 41t163.5 109.5t109 163.5t40.5 199t-40.5 199t-109 163.5t-163.5 109t-199 40.5zm0-896q-104 0-192.5 51t-140 139.5t-51.5 193t51.5 193t140 140T512 897t192.5-51.5t140-140t51.5-193t-51.5-193t-140-139.5T512 129zm192 224L573 530q-5 20-22 33.5T512 577q-21 0-38-13t-23-32L256 289v-32h32l225 181l159-117h32v32z"
                  />
                </svg>
              </button>
            )}
            {(user?.mail === "cesanchez@estudiodemoda.com.co" ||
              user?.mail === "dpalacios@estudiodemoda.com.co") && (
              <button
                className="btn btn-transparent-final btn-m"
                onClick={() => changeUser()}
                aria-label="Cambiar de rol"
            ...
            >
              ...
              >
                <path
                  d="M10 17l-1.4-1.4 3.6-3.6-3.6-3.6L10 7l5 5-5 5zM4 19h8v2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h8v2H4v14z"
                  fill="currentColor"
                />
              </svg>
              <span>Salir</span>
